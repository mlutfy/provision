    ...
    if ($filename && is_writeable(dirname($filename))) {
      ...
      if ($template = $this->load_template()) {
        ...
        $status = provision_file()->file_put_contents($filename, $this->render_template($template, $this->data))
          ->succeed('Generated config in ' . get_class($this) . ' write(): ' . (empty($this->description) ? $filename : $this->description . ' (' . $filename. ')'), 'success')
          ->fail('Could not generate in ' . get_class($this) . ' write(): ' . (empty($this->description) ? $filename : $this->description . ' (' . $filename. ')'))->status();
